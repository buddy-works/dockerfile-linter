...
let displayErrorsList = [];
let errors = [];

function findErrorDescription(code) {
    return errorsDescriptions.find((msg) => {
        if (code === msg.errorCode) {
            return true;
        }
    });
}

export function errorCode(error, displayErrorsLevels) {
    if (error === ERROR_LEVELS.INFO && displayErrorsLevels) {
        process.exit(1);
    } else if (
        error === ERROR_LEVELS.WARNING &&
        displayErrorsLevels.some(
            (level) => level === ERROR_LEVELS.WARNING || level === ERROR_LEVELS.ERROR,
        )
    ) {
        process.exit(1);
    } else if (error === ERROR_LEVELS.ERROR && displayErrorsLevels.includes(ERROR_LEVELS.ERROR)) {
        process.exit(1);
...
function getDisplayErrorsLevels(displayed) {
...
export function addError(lineNumber, code) {
    errors.push({
...
export function displayErrors(json, error) {
    for (let i = 0; i < errors.length; i++) {
        let description = findErrorDescription(errors[i].errorCode);
        displayErrorsList.push({
            lineNumber: errors[i].lineNumber,
            message: description.errorDescription,
            level: description.errorLevel,
            code: description.errorCode,
        });
    }
    if (displayErrorsList.length !== 0) {
        if (json) {
            console.log(
                JSON.stringify(
                    displayErrorsList.toSorted((a, b) => {
                        return a.lineNumber - b.lineNumber;
                    }),
                ),
            );
            errorCode(error, getDisplayErrorsLevels(displayErrorsList));
        } else {
            displayErrorsList
                .toSorted((a, b) => {
                    return a.lineNumber - b.lineNumber;
                ...
                .reduce((a, b) => {
                    if (a.length === 0) {
                        a.push({
                            line: b.lineNumber,
                            messages: [{ text: b.message, level: b.level, code: b.code }],
                        });
...
export function addDisplayError(lineNumber, errorText, level, code) {
    displayErrorsList.push({
        lineNumber: lineNumber,
        message: errorText,
        level: level,
...
export function getDisplayErrors() {
    return displayErrorsList;
}

export function cleanDisplayErrors() {
    displayErrorsList = [];
}
