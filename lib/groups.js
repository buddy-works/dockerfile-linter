...
function getArgAndEnv(commandsGroups, escape) {
    let list = [];
    let lastLineNumber =
        commandsGroups[commandsGroups.length - 1].linesNumbers[
            commandsGroups[commandsGroups.length - 1].linesNumbers.length - 1
        ];
    commandsGroups.forEach((group, index) => {
        if (
            group.instruction.toUpperCase() === 'ENV' ||
            group.instruction.toUpperCase() === 'ARG'
        ) {
    ...
    for (let i = 0; i < variables.length; i++) {
        for (let j = i + 1; j < variables.length; j++) {
            ...
            if (variables[i].validTo >= variables[j].lineNumber)
                variables[j].value = variables[j].value.replace(
                    getReplaceRegex(variables[i].name),
                    variables[i].value,
                );
        }
    }
...
export function groupLinesByInstruction(lines, escape) {
    let groups = [];
...
export function groupByCommand(rawLinesGroups) {
    let commandsGroups = [];
...
export function replaceArgAndEnv(commandsGroups, escape) {
    ...
    for (let i = 0; i < commandsGroups.length; i++) {
        if (
            commandsGroups[i].instruction.toUpperCase() !== 'ARG' &&
            commandsGroups[i].instruction.toUpperCase() !== 'ENV'
        ) {
            for (let j = 0; j < commandsGroups[i].arguments.length; j++) {
                for (let k = 0; k < argAndEnv.length; k++) {
                    if (commandsGroups[i].linesNumbers[j] <= argAndEnv[k].validTo) {
                        ...
                        commandsGroups[i].arguments[j] = commandsGroups[i].arguments[j].replace(
                            getReplaceRegex(replaceStr),
                            argAndEnv[k].value,
                        );
                    }
                }
...
export function getInlineIgnores(lines) {
    let inlineIgnores = [];
    lines.forEach((line) => {
