    ...
    groupLinesByInstruction: function (lines, escape) {
        ...
        for (let x = 0; x < filteredLines.length; x++) {
            if (!inInstruction) {
                ...
                if (
                    !instructionStart(filteredLines[x]) &&
                    instructionEnd(filteredLines, x, escape)
                ) {
                    group.push(filteredLines[x]);
...
export const { groupLinesByInstruction, groupByCommand, replaceArgAndEnv, getInlineIgnores } =
    moduleExports;
