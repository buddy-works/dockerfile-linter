import { instructionStart, instructionEnd } from './groups_helpers.js';
import { lineContainDirective, directives } from './directives_helpers.js';
import {
    regexInstruction,
    getArgumentsArrayForInstruction,
    nearestFrom,
    getArgumentsLine,
    getArgumentsArrayForCommand,
    regexCommand,
    findLineNumber,
    cmdInHealthcheck,
    isIgnored,
} from './lints_helpers.js';
...
function directiveWillBeIgnored(parsedFile) {
    ...
    for (let i = 0; i < parsedFile.lines.length; i++) {
        if (!treatedAsCommand) {
            if (
                parsedFile.lines[i].text.startsWith('#') &&
                !lineContainDirective(parsedFile.lines[i].text)
            ) {
...
function pipefailBeforeRun(parsedFile) {
    ...
    for (let i = 0; i < parsedFile.groups.length; i++) {
        if (
            parsedFile.groups[i].instruction.toUpperCase() === 'RUN' &&
            !isIgnored('ER0001', parsedFile.groups[i].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
    ...
    for (let i = 0; i < pipesUsedList.length; i++) {
        ...
        pipesLoop: for (let j = pipesUsedList[i].groupNumber; j >= 0; j--) {
...
function lastUserShouldNotBeRoot(parsedFile) {
    ...
    parsedFile.groups.forEach((group) => {
        if (
            group.instruction.toUpperCase() === 'USER' &&
            !isIgnored('EU0001', group.linesNumbers[0], parsedFile.inlineIgnores)
        ) {
    ...
    for (let i = 0; i < onlyOnceAllowed.length; i++) {
        ...
        parsedFile.groups.forEach((group, index) => {
            if (!isIgnored('EI0001', group.linesNumbers[0], parsedFile.inlineIgnores)) {
                if (
                    group.instruction.toUpperCase() === onlyOnceAllowed[i] &&
                    !cmdInHealthcheck(parsedFile, index)
                ) {
                    count++;
                    if (count > 1) {
...
function onlyArgBeforeFrom(parsedFile) {
    ...
    if (fromLineNumber !== 1) {
        for (let i = 0; i < fromGroupIndex; i++) {
            if (
                parsedFile.groups[i].instruction.toUpperCase() !== 'ARG' &&
                !isIgnored('EI0002', parsedFile.groups[i].linesNumbers[0], parsedFile.inlineIgnores)
            ) {
...
function copyFromOptionShouldReferenceToDefinedFromAlias(parsedFile) {
    for (let i = 0; i < parsedFile.groups.length; i++) {
        if (
            parsedFile.groups[i].instruction.toUpperCase() === 'COPY' &&
            !isIgnored('EC0002', parsedFile.groups[i].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
...
function copyFromOptionReferenceToOwnFromAlias(parsedFile) {
    for (let i = 0; i < parsedFile.groups.length; i++) {
        if (
            parsedFile.groups[i].instruction.toUpperCase() === 'COPY' &&
            !isIgnored('EC0001', parsedFile.groups[i].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
...
function uppercaseCheck(parsedFile) {
    for (let i = 0; i < parsedFile.groups.length; i++) {
        if (
            parsedFile.groups[i].instruction !== parsedFile.groups[i].instruction.toUpperCase() &&
            !isIgnored('EI0005', parsedFile.groups[i].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
...
function deprecatedInstruction(parsedFile) {
    ...
    for (let i = 0; i < parsedFile.groups.length; i++) {
        deprecatedInstructions.forEach((instruction) => {
            if (
                parsedFile.groups[i].instruction.toUpperCase() === instruction.name &&
                !isIgnored(
                    instruction.errorCode,
                    parsedFile.groups[i].linesNumbers[0],
...
function lintJson(parsedFile) {
    ...
    for (let i = 0; i < parsedFile.groups.length; i++) {
        for (let j = 0; j < instructions.length; j++) {
            if (
                parsedFile.groups[i].instruction.toUpperCase() === instructions[j] &&
                !isIgnored('EJ0001', parsedFile.groups[i].linesNumbers[0], parsedFile.inlineIgnores)
            ) {
                //sprawdzanie czy JSON array zostal uzyty
                if (
                    parsedFile.groups[i].arguments[0].startsWith('[') &&
                    parsedFile.groups[i].arguments[0].endsWith(']')
                ) {
...
function shouldUseJson(parsedFile) {
    ...
    for (let i = 0; i < parsedFile.groups.length; i++) {
        for (let j = 0; j < instructions.length; j++) {
            if (
                parsedFile.groups[i].instruction.toUpperCase() === instructions[j] &&
                !isIgnored('EJ0002', parsedFile.groups[i].linesNumbers[0], parsedFile.inlineIgnores)
            ) {
                if (
                    !parsedFile.groups[i].arguments[0].startsWith('[') &&
                    !parsedFile.groups[i].arguments[0].endsWith(']')
...
function mustUseJson(parsedFile) {
    for (let i = 0; i < parsedFile.groups.length; i++) {
        if (
            parsedFile.groups[i].instruction.toUpperCase() === 'SHELL' &&
            !isIgnored('EJ0003', parsedFile.groups[i].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            if (
                !parsedFile.groups[i].arguments[0].startsWith('[') &&
                !parsedFile.groups[i].arguments[0].endsWith(']')
    ...
    for (let i = parsedFile.aliases.length - 1; i >= 0; i--) {
        for (let j = i - 1; j >= 0; j--) {
            if (!isIgnored('EF0002', parsedFile.aliases[i].lineNumber, parsedFile.inlineIgnores)) {
                if (
                    parsedFile.aliases[i].name === parsedFile.aliases[j].name &&
                    parsedFile.aliases[i].name
                ) {
...
function usedTagLatest(parsedFile) {
    for (let i = 0; i < parsedFile.groups.length; i++) {
        if (
            parsedFile.groups[i].instruction.toUpperCase() === 'FROM' &&
            !isIgnored('EF0003', parsedFile.groups[i].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            //jezeli nie jest uzyta zmienna ARG albo ENV
            if (
                !parsedFile.groups[i].arguments[0].startsWith('$') &&
                parsedFile.groups[i].arguments[0].match(/:latest([^\w]|$)/)
            ) {
...
function alwaysTag(parsedFile) {
    for (let i = 0; i < parsedFile.groups.length; i++) {
        if (!isIgnored('EF0004', parsedFile.groups[i].linesNumbers[0], parsedFile.inlineIgnores)) {
            if (
                parsedFile.groups[i].instruction.toUpperCase() === 'FROM' &&
                parsedFile.groups[i].arguments[0].toLowerCase() !== 'scratch'
            ) {
                //jezeli nie jest uzyta zmienna ARG albo ENV i nie ma taga
                if (
                    !parsedFile.groups[i].arguments[0].startsWith('$') &&
                    !parsedFile.groups[i].arguments[0].match(/:/)
                ) {
                    if (parsedFile.aliases.length !== 0) {
                        ...
                        let test = parsedFile.aliases.find((alias) => {
                            return (
                                alias.name ===
                                parsedFile.groups[i].arguments[0]
                                    .match(/^[\w.\-_]+/)[0]
                                    .toLowerCase()
                            );
                        });
...
function deleteAptGetListsAfterInstall(parsedFile) {
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'RUN' &&
            !isIgnored('ER0002', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            let argumentsArrays = getArgumentsArrayForCommand(
                getArgumentsLine(parsedFile.groups[m], parsedFile.escape),
                'apt-get',
            );
...
function notUseSudo(parsedFile) {
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'RUN' &&
            !isIgnored('ER0004', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            ...
            for (let i = 0; i < usedSudo.length; i++) {
                addError(
                    findLineNumber(parsedFile.groups[m], parsedFile.escape, usedSudo[i]),
                    'ER0004',
                );
            }
        }
...
function useWorkdir(parsedFile) {
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'RUN' &&
            !isIgnored('ER0003', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
...
function lookForPointlessCommands(parsedFile) {
    ...
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'RUN' &&
            !isIgnored('ER0005', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            let line = getArgumentsLine(parsedFile.groups[m], parsedFile.escape);
...
function lookForNotRecommendedCommands(parsedFile) {
    ...
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'RUN' &&
            !isIgnored('ER0006', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            let line = getArgumentsLine(parsedFile.groups[m], parsedFile.escape);
            ...
            for (let i = 0; i < commands.length; i++) {
                ...
                for (let j = 0; j < parsedFile.groups[m].arguments.length; j++) {
                    if (
                        parsedFile.groups[m].arguments[j].match(regex) &&
                        !isIgnored(
                            'ER0006',
                            parsedFile.groups[m].linesNumbers[0],
                            parsedFile.inlineIgnores,
                        )
                    ) {
...
function usedAddForFetchPackages(parsedFile) {
    for (let i = 0; i < parsedFile.groups.length; i++) {
        if (
            parsedFile.groups[i].instruction.toUpperCase() === 'ADD' &&
            !isIgnored('EA0001', parsedFile.groups[i].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            if (
                parsedFile.groups[i].arguments[0].match(/^(http|https).+\.tar(\.xz|\.gz|\.bz2)\s/)
            ) {
...
function copyUsedForArchives(parsedFile) {
    for (let i = 0; i < parsedFile.groups.length; i++) {
        if (
            parsedFile.groups[i].instruction.toUpperCase() === 'COPY' &&
            !isIgnored('EC0003', parsedFile.groups[i].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
...
function eitherUseWgetOrCurl(parsedFile) {
    ...
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'RUN' &&
            !isIgnored('ER0007', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            //pierwsze wystapienie wget albo curl jest zapisywane,
            //przy kolejnch sprawdzane czy to samo narzedzie zostalo uzyte
            if (tool === 'wget') {
                for (let i = 0; i < parsedFile.groups[m].arguments.length; i++) {
                    let toolTemp = parsedFile.groups[m].arguments[i].match(
                        regexInstruction('curl'),
                    );
                    if (toolTemp) {
...
function useShell(parsedFile) {
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'RUN' &&
            !isIgnored('ER0008', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            let argumentsArrays = getArgumentsArrayForCommand(
                getArgumentsLine(parsedFile.groups[m], parsedFile.escape),
                'ln',
...
function useFlagY(parsedFile) {
    ...
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'RUN' &&
            !isIgnored('ER0009', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            let argumentsArrays = getArgumentsArrayForCommand(
                getArgumentsLine(parsedFile.groups[m], parsedFile.escape),
                'apt-get',
                            ...
                            for (let k = 0; k < optionsArray.length; k++) {
                                if (argumentsArrays[i].array[j] === optionsArray[k]) {
                                    ...
                                    for (let l = 0; l < shortOptionsArray.length; l++) {
                                        if (
                                            argumentsArrays[i].array[j].match(
                                                new RegExp(shortOptionsArray[l]),
                                            )
                                        ) {
                                            return;
                                        }
...
function useNoInstallRecommends(parsedFile) {
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'RUN' &&
            !isIgnored('ER0010', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            let argumentsArrays = getArgumentsArrayForCommand(
                getArgumentsLine(parsedFile.groups[m], parsedFile.escape),
                'apt-get',
            ...
            if (argumentsArrays) {
                argLoop: for (let i = 0; i < argumentsArrays.length; i++) {
...
function usedAddInsteadOfCopy(parsedFile) {
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'ADD' &&
            !isIgnored('EA0002', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            let array = getArgumentsArrayForInstruction(
                getArgumentsLine(parsedFile.groups[m], parsedFile.escape),
            );
...
function checkNumberOfArguments(parsedFile) {
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'COPY' &&
            !isIgnored('EC0004', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            let array = getArgumentsArrayForInstruction(
                getArgumentsLine(parsedFile.groups[m], parsedFile.escape),
            );
...
function useNoCache(parsedFile) {
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'RUN' &&
            !isIgnored('ER0011', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            let argumentsArrays = getArgumentsArrayForCommand(
                getArgumentsLine(parsedFile.groups[m], parsedFile.escape),
                'apk',
            ...
            if (argumentsArrays) {
                for (let i = 0; i < argumentsArrays.length; i++) {
                    if (
                        argumentsArrays[i].array.find((e) => {
                            return e === 'add';
                        })
                    ) {
                        for (let j = 0; j < argumentsArrays[i].array.length; j++) {
...
function pinVersionInAptGetInstall(parsedFile) {
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'RUN' &&
            !isIgnored('ER0012', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            let argumentsArrays = getArgumentsArrayForCommand(
                getArgumentsLine(parsedFile.groups[m], parsedFile.escape),
                'apt-get',
...
function pinVersionInPip(parsedFile) {
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'RUN' &&
            !isIgnored('ER0013', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            let argumentsArrays = getArgumentsArrayForCommand(
                getArgumentsLine(parsedFile.groups[m], parsedFile.escape),
                'pip|pip3',
...
function pinVersionInNpm(parsedFile) {
    ...
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'RUN' &&
            !isIgnored('ER0014', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            let argumentsArrays = getArgumentsArrayForCommand(
                getArgumentsLine(parsedFile.groups[m], parsedFile.escape),
                'npm',
...
function pinVersionInApkAdd(parsedFile) {
    ...
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'RUN' &&
            !isIgnored('ER0015', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            let argumentsArrays = getArgumentsArrayForCommand(
                getArgumentsLine(parsedFile.groups[m], parsedFile.escape),
                'apk',
            ...
            if (argumentsArrays) {
                for (let i = 0; i < argumentsArrays.length; i++) {
                    let apkArray = [];
                    argLoop: for (let j = 0; j < argumentsArrays[i].array.length; j++) {
...
function pinVersionInGemInstall(parsedFile) {
    ...
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'RUN' &&
            !isIgnored('ER0016', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            let argumentsArrays = getArgumentsArrayForCommand(
                getArgumentsLine(parsedFile.groups[m], parsedFile.escape),
                'gem',
            ...
            if (argumentsArrays) {
                for (let i = 0; i < argumentsArrays.length; i++) {
                    let gemsArray = [];
                    if (
                        argumentsArrays[i].array[0] === 'install' ||
                        argumentsArrays[i].array[0] === 'i'
                    ) {
                        for (let j = 1; j < argumentsArrays[i].array.length; j++) {
                            ...
                            if (!argumentsArrays[i].array[j].startsWith('-')) {
                                ...
                                argumentsArrays[i].array[j] === '-v' ||
                                argumentsArrays[i].array[j] === '--version'
                            ) {
                                gemsArray.push(argumentsArrays[i].array[j]);
    ...
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (parsedFile.groups[m].onBuild) {
            cantContain.forEach((instruction) => {
                if (
                    parsedFile.groups[m].instruction === instruction &&
                    !isIgnored(
                        'EI0004',
                        parsedFile.groups[m].linesNumbers[0],
                        parsedFile.inlineIgnores,
...
function useAbsoluteInWorkdir(parsedFile) {
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'WORKDIR' &&
            !isIgnored('EW0001', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            //jezeli nie jest ARG albo ENV
            if (
                !parsedFile.groups[m].arguments[0].startsWith('/') &&
                !parsedFile.groups[m].arguments[0].startsWith('$')
            ) {
...
function validUnixPorts(parsedFile) {
    for (let m = 0; m < parsedFile.groups.length; m++) {
        if (
            parsedFile.groups[m].instruction.toUpperCase() === 'EXPOSE' &&
            !isIgnored('EE0001', parsedFile.groups[m].linesNumbers[0], parsedFile.inlineIgnores)
        ) {
            let array = getArgumentsArrayForInstruction(
                getArgumentsLine(parsedFile.groups[m], parsedFile.escape),
            );
            array = array.reduce((ports, e) => {
