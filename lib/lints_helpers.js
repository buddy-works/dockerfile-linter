...
export function regexInstruction(instruction) {
    return new RegExp(`(^|\\s|(?<=."))${instruction}($|\\s|(?=".))`);
}

export function regexCommand(command) {
    return new RegExp(`(^${command})|[;|&]\\s*(?=${command})`);
}

export function nearestFrom(lineNumber, names) {
...
export function getArgumentsArrayForCommand(line, name, addOptions = false) {
...
export function getArgumentsLine(group, escape) {
    if (group.arguments[0].startsWith('[') && group.arguments[0].endsWith(']')) {
        ...
        for (let i = 0; i < group.arguments.length; i++) {
            if (i === 0) {
                ...
                line = line.replace(
                    new RegExp(`${fixedEscape}$`),
                    group.arguments[i].replace(/\$/g, '$$$'),
                );
            }
        }
        return line;
...
export function getArgumentsArrayForInstruction(line, addOptions = false) {
...
export function findLineNumber(group, escape, index) {
...
export function cmdInHealthcheck(parsedFile, index) {
    if (
        parsedFile.groups[index].instruction === 'CMD' &&
        parsedFile.groups[index - 1].instruction === 'HEALTHCHECK'
    ) {
        return parsedFile.groups[index - 1].arguments[
            parsedFile.groups[index - 1].arguments.length - 1
        ].endsWith(parsedFile.escape);
    } else {
        return false;
    }
}

export function isIgnored(code, line, inlineIgnores) {
    if (inlineIgnores) {
